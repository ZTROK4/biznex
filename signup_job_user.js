...
router.post("/send-email-otp-job-user", async (req, res) => {
  ...
  if (result.rows.length === 0){
    try {
        ...
        await masterPool.query('DELETE FROM job_user_verifications WHERE email = $1', [email]);
        
        await masterPool.query(
          `INSERT INTO job_user_verifications (email, email_otp, expires_at)
           VALUES ($1, $2, $3)
           ON CONFLICT (email) 
...
router.post("/send-phone-otp-job-user", async (req, res) => {
    ...
    try {
      ...
      await masterPool.query(
        `UPDATE job_user_verifications 
         SET phone = COALESCE(phone, $1), phone_otp = $2, expires_at = $3, is_phone_verified = FALSE 
         WHERE email = $4;`,
...
router.post("/verify-email-otp-job-user", async (req, res) => {
  ...
  try {
    ...
    await masterPool.query(`UPDATE job_user_verifications SET is_email_verified = TRUE WHERE email = $1;`, [email]);
...
router.post("/verify-phone-otp-job-user", async (req, res) => {
  ...
  try {
    ...
    await masterPool.query(`UPDATE job_user_verifications SET is_phone_verified = TRUE WHERE email = $1;`, [email]);
